...
function buildSupabaseUrl(req) {
  ...
  const url = new URL(req.url, `https://${req.headers.host || "localhost"}`);
  const limit = toInteger(url.searchParams.get("limit"), 500, 1, 2000);
  ...
  endpoint.searchParams.set("event_name", "eq.email_subscribe");
...
function normalizeRows(rows) {
  return rows
    .filter((row) => row && row.result_target)
    .map((row) => ({
      registered_at: row.occurred_at || "",
      email: cleanText(row.result_target, 254),
...
function buildUniqueRows(rows) {
  const grouped = new Map();
  rows.forEach((row) => {
    const key = row.email.toLowerCase();
    const current = grouped.get(key);
    if (!current) {
      grouped.set(key, { ...row, registrations: 1, first_registered_at: row.registered_at });
      return;
    }

    current.registrations += 1;
    current.first_registered_at = row.registered_at < current.first_registered_at ? row.registered_at : current.first_registered_at;
  });
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const uniqueRows = buildUniqueRows(rows);
    ...
    send(res, 500, { ok: false, error: "Failed to load email subscribers" });
  }
};
