...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!question) {
      send(res, 400, { ok: false, error: "Question is required" });
      return;
    }

    const sessionId = cleanText(body.chat_session_id, 120) || `chat_${Date.now()}`;
    const now = new Date().toISOString();
    const event = {
      event_id: `chat_${sessionId}_${Date.now()}_${Math.random().toString(16).slice(2)}`,
      event_name: "chatbot_message",
      occurred_at: now,
      lp_variant: cleanText(body.lp_variant, 80) || "control",
      cta_location: cleanText(body.cta_location, 80) || "chatbot",
      section_id: cleanText(body.answer_title, 120),
      result_target: question,
      page_path: cleanText(body.page_path, 240),
      page_url: cleanText(body.page_url, 600),
      referrer: cleanText(body.answer_text, 600),
      ...
      creative: cleanText(body.answer_text, 180),
    ...
    send(res, 500, { ok: false, error: "Chatbot log failed" });
  }
};
